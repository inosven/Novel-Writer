...
- 说明一行，可为空行省略。
- 服务写回时整文件重写，保留块内未识别的行（放回该批注块末尾），不改块顺序；块外内容不保留。
...
- `notes.md` 里批注块内不认识的行：原样保留在该块末尾；`## A…` 块之外的内容（除首行标题）写回时不保留，所以不要在块外手写内容。
...
在 `~/tmp/novel-e2e-sanguo`（《三顾》，5 章正文，账本截至第 5 章）上，由主会话用 Chrome 操作界面、用 `claude -p` 跑命令：

- `reader.sh start 8765` 启动，界面显示书名、6 章计划、5 章列表（字数不含标点：3358/3685/4452/2960/5125）、已定稿和已审稿标记。
- 打开第 4 章：审稿条目蓝色高亮出现在正文。选中"俺兄长"点"批注"，填说明保存：`notes.md` 生成，A1 位置为 `第4章「过俺兄长」`（选区在本章出现 3 次，服务扩展到唯一），正文黄色高亮。
- 选中"俺"点"查找"，勾选"账本"：第 4 章 6 处、第 5 章 6 处、`bible/state.md` 1 处。勾选第 5 章两处和账本一处，加入 A1：位置变 4 条，切到第 5 章有两处黄色高亮。
- 第 5 章审稿页签给 N1 标"未处理"，`reviews/Chapter-05.md` 该条目末尾多一行 `- 未处理：作者决定保留`。
- A1 改"作废"后高亮变虚线，改回"未处理"恢复；删掉账本那处位置；在"其他 .md 文件路径"输入 `characters/张飞.md` 回车能打开。
- `rollback.sh 5` 后 `/novel:revise 5`：编辑把 A1 在第 5 章的两处"俺"改回"我"，`notes.md` 对应位置行加了 ` 已处理`，第 4 章位置未动、批注状态仍"未处理"；复审的上下文包含"作者批注"段。`/novel:status` 输出"未处理批注：1 条（A1……）"。
- 发现并在终审修复：引用唯一化会产出"，俺"这类过短锚点（加最短 8 字）；编辑标 ` 未处理：原文已变` 的位置行原先解析不出。
