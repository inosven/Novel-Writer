/**
 * @module src/core/agents/PlannerAgent
 * @description 策划 Agent — 负责对话式故事规划。
 * 支持任务类型：需求收集(collect_requirements)、大纲生成(generate_outline_draft)、
 * 大纲优化(refine_outline)、角色建议(suggest_characters)、角色设计(design_character)、
 * 影响分析(analyze_impact)。
 * 通过多轮对话逐步收集创作意图，待用户确认后生成完整大纲。
 */
...
`;
    ...
    // Extract metadata JSON if present (```json...``` block)
    const jsonMatch = response.match(/```json\s*([\s\S]*?)```/);
    const metadata = jsonMatch ? this.extractJSON<OutlineDraftMeta>(jsonMatch[0]) : null;

    // Extract outline content:
    // 1. Remove JSON metadata blocks
    // 2. Remove wrapping markdown code fences
    // 3. Keep everything else as the outline
    let outlineContent = response;

    // Remove ```json...``` blocks (metadata only)
    outlineContent = outlineContent.replace(/```json[\s\S]*?```/g, '').trim();

    // Remove leading/trailing ``` markers if the whole response is wrapped in markdown fence
    outlineContent = outlineContent.replace(/^```(?:markdown)?\s*\n?/, '').replace(/\n?```\s*$/, '').trim();

    console.log('[PlannerAgent.generateOutlineDraft] Outline content length after cleanup:', outlineContent.length);
    console.log('[PlannerAgent.generateOutlineDraft] Has metadata:', !!metadata);

    // Safety: if cleanup stripped too much, fall back to raw response
    if (outlineContent.length < 50 && response.length > 50) {
      console.log('[PlannerAgent.generateOutlineDraft] Cleanup stripped too much, using raw response');
