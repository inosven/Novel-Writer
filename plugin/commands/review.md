...
2. 运行 `${CLAUDE_PLUGIN_ROOT}/scripts/context.sh N review`，把 stdout 保存到一个临时文件（会话的 scratchpad 目录或 `mktemp`）。退出码非零时把 stderr 给用户，停止。不要把上下文包读进主对话。
3. 用 novel:reviewer 子代理审稿。提示词第一行"审稿第 N 章。"，第二行是小说目录的绝对路径，第三行是上下文包文件的绝对路径，并要求它先用 Read 把该文件完整读完再审。
