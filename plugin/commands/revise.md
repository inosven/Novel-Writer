...
4. 运行 `${CLAUDE_PLUGIN_ROOT}/scripts/model.sh editor "模型覆盖串"` 取模型名。输出是 `inherit` 就不传 `model` 参数；否则调用子代理时把它作为 `model` 参数传入。
5. 用 novel:editor 子代理修改。提示词：
   - 按报告修改时："修订第 N 章。按 reviews/Chapter-NN.md 处理以下条目：C1, C2, M1。"
   - 按说明修改时："修订第 N 章。用户要求：（修改说明原文）。"
6. 子代理返回后，记下它给出的改动前后对照。
7. 复审：读 `${CLAUDE_PLUGIN_ROOT}/commands/review.md`，按其步骤 2 到 5 执行一遍（其 model.sh 那步用同一个模型覆盖串；旧报告归档为 `Chapter-NN.vK.md`，重新生成 `reviews/Chapter-NN.md`）。
8. 把第 6 步的前后对照和复审的汇总行、critical 列表一起给用户。复审后 critical 为 0 就加一句：可以 `/novel:finalize N` 定稿；仍有 critical 就加一句：可以再次 `/novel:revise N`。
