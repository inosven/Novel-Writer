...
上下文包里有"上一版审稿报告"段时，这是复审，不是重审。规则：

- 上一版的每条问题逐条核对。标了"已处理"的，看正文是否真改好了：改好了就不再列出；没改好或改出新问题就保留原编号重新写。
- 标了"未处理"的是作者的决定，不再提出，不计入汇总。
- 仍然存在且没标记的，保留原编号和原严重度，不要因为换了措辞就升级或降级。严重度只在正文相关段落改变了的情况下才能变。
- 新发现的问题接在该级别原有最大编号之后（上一版 major 到 M3，新问题从 M4 起）。
- 汇总行下面加一行"较上一版：已解决 X 条，保留 Y 条，新增 Z 条"。
