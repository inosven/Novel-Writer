...
See [README.md](README.md) (Chinese) for installation and commands. Commands: `/novel:init`, `/novel:plan`, `/novel:write N`, `/novel:review N`, `/novel:revise N`, `/novel:finalize N`, `/novel:status`, `/novel:auto N` (write/review/revise/finalize chapter after chapter, stopping only when a critical issue survives re-review). Per-agent models are set in `novel.yaml` (`models:`); append a model name or `role=model` list after the chapter number to override for one run, e.g. `/novel:auto 8 writer=opus,reviewer=haiku`.
